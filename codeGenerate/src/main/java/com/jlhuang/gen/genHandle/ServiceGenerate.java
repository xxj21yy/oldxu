...
public class ServiceGenerate implements Generate {
    ...
    public void proess(String modelPath, Filer filer, VelocityContext context, Table table) {
        modelPath = Message.SERVICE_VM_PATH;
        System.out.println("start made pojo");
        String service_package = (String) context.get("service_package");
        String base_path = (String)context.get("base_path");

        String model = table.getCamelTableName();
        String outPath = base_path + "/src/main/java/" + service_package.replace(".", "/") + "/" + model + "Service.java";
